...
const DashboardCharts = dynamic(
  ...
  { ssr: false, loading: () => <div className="h-[350px] w-full bg-muted/20 animate-pulse rounded-xl" /> }
);
...
export default function DashboardPage() {
  ...
  const categoryChartData = Object.entries(categoryExpenses).map(([category, amount]) => {
  ...
  const budgetProgress = budgets.map((b) => {
    ...
    if (spent > b.limit_amount) {
      budgetAlerts.push(`You have exceeded your ${b.category} budget limit by ${formatCurrency(spent - b.limit_amount, currency)}!`);
    } else if (percentage >= 85) {
      budgetAlerts.push(`Warning: You have used ${percentage.toFixed(0)}% of your ${b.category} budget.`);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Monitor your income, expenses, and budget limits.</p>
        </div>
        ...
        <div className="flex items-center gap-1.5 self-start sm:self-auto">
          ...
          <Dialog open={isAddTxOpen} onOpenChange={setIsAddTxOpen}>
            <DialogTrigger asChild>
              <Button size="sm" className="flex items-center gap-1 shadow-sm font-semibold">
                <Plus size={16} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-border bg-card shadow-sm">
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-base sm:text-lg">Monthly Budgets</CardTitle>
              <CardDescription>Limit spent tracking</CardDescription>
            </div>
            <Dialog open={isSetBudgetOpen} onOpenChange={setIsSetBudgetOpen}>
              <DialogTrigger asChild>
                <Button size="sm" variant="outline" className="h-8 px-2.5 font-medium border-border">
              ...
              <DialogContent className="max-w-md">
                ...
                <BudgetForm month={activeMonth} onSuccess={() => setIsSetBudgetOpen(false)} />
              </DialogContent>
            </Dialog>
          </CardHeader>
          <CardContent className="space-y-4">
            {budgetProgress.length === 0 ? (
              <div className="py-8 text-center text-xs text-muted-foreground flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-slate-500">Configure category budgets to avoid overspending.</p>
              </div>
            ) : (
                    ...
                    <div key={item.id} className="space-y-1.5 text-xs">
                      <div className="flex justify-between items-center font-medium">
                        ...
                        <span className="text-muted-foreground">
                          <span className={`font-semibold ${isOver ? 'text-destructive' : isNear ? 'text-amber-500' : 'text-foreground'}`}>
      ...
      <Card className="border-border bg-card shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <div>
            ...
            <CardDescription>Last transactions logged this month</CardDescription>
          </div>
